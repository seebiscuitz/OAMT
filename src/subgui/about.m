...
% Last Modified by GUIDE v2.5 17-Feb-2016 15:14:21
...
% - set about information -
    aboutstring=strcat('About:  The Ocean Acoustics Modelling Toolbox (OAMT) is a MATLAB graphical user interface',...
    'designed for the purpose of simulating cumulative sound exposure in a 3D underwater environment',...
    'from multiple noise sources. This software was created as final year MEng project.',...
    '');
    set(handles.text2,'string',aboutstring);
    % - get version number -
    list=dir([cd '\oamt*.m']);
    if ~isempty(list)
        indx=strfind(list.name,'.');
        version=list.name(1:indx-1);
    else
        version='error';
    end
    set(handles.text4,'string',version);
    % - end -
    % - set latest change -
    %set(handles.text5,'string','Major Updates: ');
    % - end - 
% - end -
